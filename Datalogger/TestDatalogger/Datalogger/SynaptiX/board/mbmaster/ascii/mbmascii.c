...
STATIC BOOL
bMBMSerialASCIITransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE *ubValue );
#elif MBM_SERIAL_API_VERSION == 2
STATIC BOOL
bMBMSerialASCIITransmitterEmptyAPIV2CB( xMBHandle xHdl, UBYTE *pubBufferOut, USHORT usBufferMax,
...
void
vMBMLogASCIIFrame( eMBPortLogLevel eLevel, xMBMInternalHandle *pxIntHdl, char *szMsg, const UBYTE *pubPayload,
                   USHORT usLength );
#endif
                     ...
                     UCHAR ucStopBits )
{
...
#endif
    {
        ...
        if( NULL != pxFrameHdl )
        {
            ...
            if( MB_ENOERR != ( eStatus2 = eMBPSerialInit( &( pxFrameHdl->xSerHdl ), ucPort, ulBaudRate, 7, eParity,
                                                          ucStopBits, pxIntHdl
                                                          ...
                                                          ) ) )
            {
                eStatus = eStatus2;
                                                              ...
                                                              bMBMSerialASCIITimerCB, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
#if MBM_ASCII_BACKOF_TIME_MS > 0
                                                              ...
                                                              bMBMSerialASCIIBackOffTimerCB, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
                     ...
                     ( eStatus2 = eMBPTimerInit( &( pxFrameHdl->xWaitTmrHdl ),
                                                 MBM_SERIAL_ASCII_DYNAMIC_WAITAFTERSEND_TIMEOUT_MS( ulBaudRate ),
...
#endif
        if( MB_IS_VALID_HDL( pxASCIIHdl, xMBMASCIIFrameHdl ) && MB_IS_VALID_WRITE_ADDR( ucSlaveAddress ) &&
            ( usMBPDULength <= ( MBM_SER_PDU_SIZE_MAX - ( 1 /* Slave Address */ + 1 /* LRC */ ) ) ) )
        {
            MBP_ENTER_CRITICAL_SECTION();
            ...
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_ASCII ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_ASCII,
                             ...
                             ( USHORT )pxASCIIHdl->ubIdx, ( USHORT )pxASCIIHdl->eRcvState, pxASCIIHdl->usRcvBufferPos,
                             ( USHORT )pxASCIIHdl->eSndState, pxASCIIHdl->usSndBufferCnt );
            }
#endif
            MBP_EXIT_CRITICAL_SECTION();
            ...
            ubLRC =
                ubMBMSerialASCIILRC( ( UBYTE * )&( pxASCIIHdl->ubASCIIFrameBuffer[0] ), pxASCIIHdl->usSndBufferCnt );
            ...
            if( MB_ENOERR !=
                ( eStatus = eMBPSerialTxEnable(
                      pxASCIIHdl->xSerHdl, ( pbMBPSerialTransmitterEmptyCB )bMBMSerialASCIITransmitterEmptyAPIV1CB , pxIntHdl ) ) )
            {
            ...
            if( MB_ENOERR !=
                ( eStatus = eMBPSerialTxEnable(
                      pxASCIIHdl->xSerHdl, ( pbMBPSerialTransmitterEmptyCB )bMBMSerialASCIITransmitterEmptyAPIV2CB , pxIntHdl ) ) )
            {
                HDL_RESET_TX( pxASCIIHdl );
            }
#else
            ...
            else
            {
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_ASCII ) )
                {
                    ...
                    vMBMLogASCIIFrame( MB_LOG_DEBUG, pxIntHdl, "Sending frame: ",
                                       ( const UBYTE * )&pxASCIIHdl->ubASCIIFrameBuffer[MBM_SER_PDU_ADDR_OFF],
                                       pxASCIIHdl->usSndBufferCnt );
#else
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_ASCII,
        ...
        if( MB_IS_VALID_HDL( pxASCIIHdl, xMBMASCIIFrameHdl ) && MB_IS_VALID_READ_ADDR( ucSlaveAddress ) )
        {
            if( NULL == pusMBPDULength )
            {
                ...
                xAnalyzerFrame.eFrameType = MB_FRAME_DAMAGED;
                     ...
                     ( ubMBMSerialASCIILRC( ( UBYTE * )&( pxASCIIHdl->ubASCIIFrameBuffer[0] ),
                                            pxASCIIHdl->usRcvBufferPos ) == 0 ) )
            {
                *pusMBPDULength =
                    ( USHORT )( pxASCIIHdl->usRcvBufferPos - ( MBM_SER_PDU_PDU_OFF + MBM_SER_PDU_SIZE_LRC ) );
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_ASCII ) )
                {
#if defined( MBM_ENABLE_DEBUG_FACILITY_HEAVY ) && ( MBM_ENABLE_DEBUG_FACILITY_HEAVY == 1 )
            ...
            if( MB_ENOERR != ( eStatus2 = eMBPSerialTxEnable( pxASCIIHdl->xSerHdl, NULL, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
            if( MB_ENOERR != ( eStatus2 = eMBPSerialRxEnable( pxASCIIHdl->xSerHdl, NULL , pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
            /* This timer must be disabled as well as the master request could run in a slave timeout.
             * If the timer is still running it would trigger for example during the transmission of
             * the next frame.
             */
            if( MB_ENOERR != ( eStatus2 = eMBPTimerStop( pxASCIIHdl->xTmrHdl ) ) )
            {
                eStatus = eStatus2;
            }
...
vMBMSerialASCIIReceiverAPIV1CB( xMBHandle xHdl, UBYTE ubValue )
{
    ...
    switch( pxASCIIFrameHdl->eRcvState )
    {
            ...
            else
            {
                ...
                switch( pxASCIIFrameHdl->eBytePos )
                {
                    ...
                    case BYTE_LOW_NIBBLE:
                        /* because of the |= ubBinValue we access usRcvBufferPos twice, it's volatile and may have
                         * change, thus use temp var here */
                        usRcvBufferPos = pxASCIIFrameHdl->usRcvBufferPos;
                        pxASCIIFrameHdl->ubASCIIFrameBuffer[usRcvBufferPos] |= ubBinValue;
        ...
        case STATE_RX_WAIT_EOF:
            ...
            {
                bEnableTimer = FALSE;
                eStatus = eMBPSerialRxEnable( pxASCIIFrameHdl->xSerHdl, NULL , pxIntHdl );
...
bMBMSerialASCIITransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE *pubValue )
{
    ...
    if( bEnableRx )
    {
        ...
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxASCIIFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB )vMBMSerialASCIIReceiverAPIV1CB ) , pxIntHdl )
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
                                        ...
                                        USHORT *pusBufferWritten )
{
    ...
    *pusBufferWritten = 0;
    ...
    if( bEnableRx )
    {
        HDL_RESET_RX( pxASCIIFrameHdl );
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxASCIIFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB )vMBMSerialASCIIReceiverAPIV2CB , pxIntHdl ) )
        {
