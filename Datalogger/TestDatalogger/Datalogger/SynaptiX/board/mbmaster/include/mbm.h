...
/*! \brief A struct to hold the configuration of a MODBUS MASTER stack. */
typedef struct xMBMaster xMBMaster;
typedef xMBMaster* pxMBMaster;
struct xMBMaster {
	xMBMHandle xMBMHdl;
	void *xSerHdl;
    void *xTmrHdl;
    void *xTmrTimeoutHdl;
    void *xTmr_BackOffHdl;
#if MBM_RTU_WAITAFTERSEND_ENABLED == 1
    void *xTmrWaitHdl;
#endif    
	// eMB_Config_t config;
#if MBM_SERIAL_API_VERSION == 1
	BOOL( *pvMBPReceiveFN ) ( xMBHandle xHdl, UBYTE ubValue );
	BOOL( *pbMBPTransmitterEmptyFN ) ( xMBHandle xHdl, UBYTE *pubValue );
#endif
#if MBM_SERIAL_API_VERSION == 2
    BOOL( *pvMBPReceiveFN ) ( xMBHandle xHdl, const UBYTE *pubBufferIn,
                                USHORT usBufferLen );
	BOOL( *pbMBPTransmitterEmptyFN ) ( xMBHandle xHdl, UBYTE *pubBufferOut,
                                        USHORT usBufferMax, USHORT *pusBufferWritten );
#endif

	BOOL(*rs485_de_select)();
	BOOL(*rs485_de_deselect)();

    volatile UBYTE			ubRxData;
    volatile UBYTE			ubTxBuff;
...
/*! \brief a subrequest for Read File Record (0x14)
 */
typedef struct
{
...
/*! \brief a subrequest for Write File Record (0x15)
 */
typedef struct
{
    USHORT usFileNumber;   /*!< File number. 0x0001 - 0xFFFF */
 ...
 */
_DLLEXP eMBErrorCode
eMBMReadHoldingRegisters( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
 ...
 *  as following. New Content = (Current Content AND usANDMask) OR (usORMask AND (NOT usANDMask))
 *
 * \param xHdl A valid MODBUS master handle.
 * \param ucSlaveAddress Slave address. Note that a broadcast address is not
 *   allowed for a function which expects a response.
 ...
 * \return eMBErrorCode::MB_ENOERR if the slave responded within the timeout
 *   and the the reply conforms to the MODBUS protocol specification. If the slave
 *   did not respond within the timeout the
 *   function returns eMBErrorCode::MB_ETIMEDOUT. Any other errors are IO
 *   errors.
 */
_DLLEXP eMBErrorCode
 ...
 */
_DLLEXP eMBErrorCode
eMBMWriteSingleRegister( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegAddress, USHORT usValue );
#endif
 ...
 */
_DLLEXP eMBErrorCode
eMBMReadInputRegisters( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
                        USHORT arusBufferOut[] );
#endif
 ...
 */
_DLLEXP eMBErrorCode
eMBMWriteMultipleRegisters( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
 ...
 */
_DLLEXP eMBErrorCode
eMBMReadWriteMultipleRegisters( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usWriteRegStartAddress,
                                UBYTE ubWriteNRegs, const USHORT arusBufferIn[], USHORT usReadRegStartAddress,
                                UBYTE ubReadNRegs, USHORT arusBufferOut[] );
#endif
 ...
 */
_DLLEXP eMBErrorCode
eMBMReadWriteRAWPDU( xMBMHandle xHdl, UCHAR ucSlaveAddress, UCHAR ucFunctionCode, const UBYTE arubPayloadIn[],
                     UBYTE ubPayloadInLength, UBYTE arubPayloadOut[], UBYTE ubPayloadOutLengthMax,
 ...
 */
_DLLEXP eMBErrorCode
eMBMReadFileRecord( xMBHandle xHdl, UCHAR ucSlaveAddress, const xMBMFileSubReadReq_t arxSubRequests[],
 ...
 */
_DLLEXP eMBErrorCode
eMBMWriteFileRecord( xMBHandle xHdl, UCHAR ucSlaveAddress, const xMBMFileSubWriteReq_t arxSubRequests[],
 ...
 */
_DLLEXP void
vMBMReadHoldingRegistersPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
 ...
 */
_DLLEXP void
vMBMWriteSingleRegisterPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegAddress, USHORT usValue,
 ...
 */
_DLLEXP void
vMBMReadInputRegistersPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
                              USHORT arusBufferOut[], eMBMQueryState *peState, eMBErrorCode *peStatus );
#endif
 ...
 */
_DLLEXP void
vMBMWriteMultipleRegistersPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
 ...
 */
_DLLEXP void
vMBMReadWriteMultipleRegistersPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usWriteRegStartAddress,
                                      UBYTE ubWriteNRegs, const USHORT arusBufferIn[], USHORT usReadRegStartAddress,
 ...
 */
_DLLEXP void
vMBMReadWriteRAWPDUPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, UCHAR ucFunctionCode, const UBYTE arubPayloadIn[],
                           UBYTE ubPayloadInLength, UBYTE arubPayloadOut[], UBYTE ubPayloadOutLengthMax,
                           UBYTE *pubPayloadOutLength, eMBMQueryState *peState, eMBErrorCode *peStatus );
 ...
 */
_DLLEXP void
vMBMReportSlaveID( xMBHandle xHdl, UCHAR ucSlaveAddress, UBYTE arubBufferOut[], UBYTE ubBufferMax, UBYTE *pubLength,
                   eMBMQueryState *peState, eMBErrorCode *peStatus );
#endif
 ...
 */
_DLLEXP void
vMBMReadFileRecordPolled( xMBHandle xHdl, UCHAR ucSlaveAddress, const xMBMFileSubReadReq_t arxSubRequests[],
                          xMBMFileSubReadResp_t arxSubResponses[], USHORT usNSubRequests, eMBMQueryState *peState,
                          eMBErrorCode *peStatus );
#endif
 ...
 */
_DLLEXP void
vMBMWriteFileRecordPolled( xMBHandle xHdl, UCHAR ucSlaveAddress, const xMBMFileSubWriteReq_t arxSubRequests[],
 ...
 */
_DLLEXP eMBErrorCode
eMBMSerialInit( pxMBMaster pxMBHdl, eMBSerialMode eMode, UCHAR ucPort, ULONG ulBaudRate, eMBSerialParity eParity );
 ...
 */
_DLLEXP eMBErrorCode
eMBMSerialInitExt( pxMBMaster pxMBHdl, eMBSerialMode eMode, UCHAR ucPort, ULONG ulBaudRate, eMBSerialParity eParity,
                   UCHAR ucStopBits );
#endif
 ...
 */
_DLLEXP eMBErrorCode
eMBMUDPSetSlave( xMBMHandle xHdl, const CHAR *pcUDPClientAddress, USHORT usUDPSlavePort );
