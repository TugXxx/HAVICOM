...
#define MBM_FUNC_MASK_WR_REG               ( 0x16 )
#define MBM_FUNC_MASK_WR_REG_RESP_SIZE     ( 7 )
#define MBM_FUNC_MASK_WR_REG_RESP_ADDR_OFF ( MB_PDU_DATA_OFF )
#define MBM_FUNC_MASK_WR_REG_RESP_AND_OFF  ( MBM_FUNC_MASK_WR_REG_RESP_ADDR_OFF + 2 )
#define MBM_FUNC_MASK_WR_REG_RESP_OR_OFF   ( MBM_FUNC_MASK_WR_REG_RESP_AND_OFF + 2 )
...
#define MBM_FUNC_RDWR_MUL_REGS_RESP_SIZE( nRegs ) ( USHORT )( 1 + 1 + 2 * ( nRegs ) )
...
#if MBM_FUNC_MASK_WR_REG_ENABLED == 1
void
vMBMMaskWriteRegisterPolled( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegAddress, USHORT usANDMask,
                             USHORT usORMask, eMBMQueryState *peState, eMBErrorCode *peStatus )
{
    USHORT usRegAddressWritten;
    USHORT usANDMaskWritten;
    USHORT usORMaskWritten;
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                {
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                 ...
                                 "] Mask write single register request (start=" MBP_FORMAT_USHORT
                                 ", AND mask=" MBP_FORMAT_USHORT ", OR mask=" MBP_FORMAT_USHORT ").\n",
                                 ( USHORT )pxIntHdl->ubIdx, usRegAddress, usANDMask, usORMask );
                }
#endif
                pxIntHdl->usFrameMBPDULength = 0;
                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] = MBM_FUNC_MASK_WR_REG;
                ...
                pxIntHdl->usFrameMBPDULength++;
                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] = ( UBYTE )( usANDMask >> 8U );
                pxIntHdl->usFrameMBPDULength++;
                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] = ( UBYTE )( usANDMask & 0x00FFU );
                pxIntHdl->usFrameMBPDULength++;
                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] = ( UBYTE )( usORMask >> 8U );
                pxIntHdl->usFrameMBPDULength++;
                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] = ( UBYTE )( usORMask & 0x00FFU );
                pxIntHdl->usFrameMBPDULength++;
            ...
            case MBM_STATE_DISASSEMBLE:
                *peState = MBM_STATE_DONE;
                if( ( MBM_FUNC_MASK_WR_REG_RESP_SIZE == pxIntHdl->usFrameMBPDULength ) &&
                    ( MBM_FUNC_MASK_WR_REG == pxIntHdl->pubFrameMBPDUBuffer[MB_PDU_FUNC_OFF] ) )
                {
                    /* verify the response if it matches our query */
                    usRegAddressWritten =
                    ...
                    usANDMaskWritten =
                        ( USHORT )( ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_MASK_WR_REG_RESP_AND_OFF] << 8U );
                    usANDMaskWritten |= ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_MASK_WR_REG_RESP_AND_OFF + 1];
                    usORMaskWritten =
                        ( USHORT )( ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_MASK_WR_REG_RESP_OR_OFF] << 8U );
                    usORMaskWritten |= ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_MASK_WR_REG_RESP_OR_OFF + 1];
                    if( ( usRegAddressWritten == usRegAddress ) && ( usANDMaskWritten == usANDMask ) &&
                        ( usORMaskWritten == usORMask ) )
                    {
                        *peStatus = MB_ENOERR;
                    }
                    else
        ...
        if( MBM_STATE_DONE == *peState )
        {
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                             "[IDX=" MBP_FORMAT_USHORT
                             "] Mask write single register finished. Status: " MBP_FORMAT_USHORT "\n",
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                {
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                 "[IDX=" MBP_FORMAT_USHORT
                                 "] Write single holding register request (start=" MBP_FORMAT_USHORT ").\n",
        ...
        if( MBM_STATE_DONE == *peState )
        {
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                             "[IDX=" MBP_FORMAT_USHORT
                             "] Write single holding register finished. Status: " MBP_FORMAT_USHORT "\n",
                             ( USHORT )pxIntHdl->ubIdx, ( USHORT )*peStatus );
            }
        }
#endif
                                ...
                                USHORT arusBufferOut[], eMBMQueryState *peState, eMBErrorCode *peStatus )
{
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                if( ( ( ( ULONG )usRegStartAddress + ( ULONG )ubNRegs ) < ( ULONG )0x10000 ) && ( ubNRegs > 0 ) &&
                    ( ubNRegs <= MBM_FUNC_RD_REGS_REGCNT_MAX ) )
                {
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     "[IDX=" MBP_FORMAT_USHORT
                                     "] Read holding registers request (start=" MBP_FORMAT_USHORT
                                     ", length=" MBP_FORMAT_USHORT ").\n",
                                     ( USHORT )pxIntHdl->ubIdx, usRegStartAddress, ( USHORT )ubNRegs );
                    }
#endif
                    pxIntHdl->usFrameMBPDULength = 0;
...
void
vMBMWriteMultipleRegistersPolled( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
                                  const USHORT arusBufferIn[], eMBMQueryState *peState, eMBErrorCode *peStatus )
{
    USHORT              usRegAddressWritten;
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
#if MBM_ENABLE_FULL_API_CHECKS == 1
                if( ( ( ( ULONG )usRegStartAddress + ( ULONG )ubNRegs ) < 0x10000UL ) && ( ubNRegs > 0 ) &&
                    ( ubNRegs <= MBM_FUNC_WR_MUL_REGCNT_MAX ) )
#else
                if( TRUE )
#endif
                {
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     "[IDX=" MBP_FORMAT_USHORT
                                     "] Write multiple holding registers request (start=" MBP_FORMAT_USHORT
                                     ", length=" MBP_FORMAT_USHORT ").\n",
                                     ( USHORT )pxIntHdl->ubIdx, usRegStartAddress, ( USHORT )ubNRegs );
                    }
#endif
            ...
            case MBM_STATE_DISASSEMBLE:
                *peState = MBM_STATE_DONE;
                ...
                {
                    usRegAddressWritten =
                        ( USHORT )( ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_MUL_REGS_RESP_ADDR_OFF] << 8U );
                    usRegAddressWritten |=
                        ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_MUL_REGS_RESP_ADDR_OFF + 1];
        ...
        if( MBM_STATE_DONE == *peState )
        {
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                             "[IDX=" MBP_FORMAT_USHORT
                             "] Write multiple holding registers finished. Status: " MBP_FORMAT_USHORT "\n",
                             ( USHORT )pxIntHdl->ubIdx, ( USHORT )*peStatus );
            }
        }
#endif
...
void
vMBMReadWriteMultipleRegistersPolled( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usWriteRegStartAddress,
                                      UBYTE ubWriteNRegs, const USHORT arusBufferIn[], USHORT usReadRegStartAddress,
                                      UBYTE ubReadNRegs, USHORT arusBufferOut[], eMBMQueryState *peState,
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                if( MB_IS_VALID_READ_ADDR( ucSlaveAddress ) &&
                    ...
                    ( ( ( ULONG )usReadRegStartAddress + ( ULONG )ubReadNRegs ) < 0x10000UL ) && ( ubReadNRegs > 0 ) &&
                    ( ubReadNRegs <= MBM_FUNC_RDWR_MUL_REGS_REGCNT_RD_MAX ) )
                {
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     ...
                                     ", write length=" MBP_FORMAT_USHORT ", read start=" MBP_FORMAT_USHORT
                                     ", read length=" MBP_FORMAT_USHORT ").\n",
                                     ( USHORT )pxIntHdl->ubIdx, usWriteRegStartAddress, ( USHORT )ubWriteNRegs,
                                     usReadRegStartAddress, ( USHORT )ubReadNRegs );
                    }
#endif
                    pxIntHdl->usFrameMBPDULength = 0;
        ...
        if( MBM_STATE_DONE == *peState )
        {
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                             "[IDX=" MBP_FORMAT_USHORT
                             "] Write/read holding multiple registers done. Status: " MBP_FORMAT_USHORT "\n",
                             ( USHORT )pxIntHdl->ubIdx, ( USHORT )*peStatus );
            }
        }
#endif
...
#if MBM_FUNC_MASK_WR_REG_ENABLED == 1
eMBErrorCode
eMBMMaskWriteRegister( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegAddress, USHORT usANDMask, USHORT usORMask )
{
    eMBErrorCode   eStatus;
    eMBMQueryState eState = MBM_STATE_NONE;
    do
    {
        vMBMMaskWriteRegisterPolled( xHdl, ucSlaveAddress, usRegAddress, usANDMask, usORMask, &eState, &eStatus );
...
#if MBM_FUNC_RD_HOLDING_ENABLED == 1
eMBErrorCode
eMBMReadHoldingRegisters( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
                          USHORT arusBufferOut[] )
{
    ...
    do
    {
        vMBMReadHoldingRegistersPolled( xHdl, ucSlaveAddress, usRegStartAddress, ubNRegs, arusBufferOut, &eState,
                                        &eStatus );
        vTaskDelay( 5 ); // Optional delay to allow processing
    } while( eState != MBM_STATE_DONE );
    return eStatus;
}
#endif
...
#if MBM_FUNC_WR_MUL_REGS_ENABLED == 1
eMBErrorCode
eMBMWriteMultipleRegisters( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
...
eMBMReadWriteMultipleRegisters( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usWriteRegStartAddress, UBYTE ubWriteNRegs,
                                const USHORT arusBufferIn[], USHORT usReadRegStartAddress, UBYTE ubReadNRegs,
                                USHORT arusBufferOut[] )
{
    eMBErrorCode   eStatus;
    eMBMQueryState eState = MBM_STATE_NONE;
    ...
    do
    {
        vMBMReadWriteMultipleRegistersPolled( xHdl, ucSlaveAddress, usWriteRegStartAddress, ubWriteNRegs, arusBufferIn,
