...
STATIC xMBMInternalHandle *
       pxMBMGetNewHdl( pxMBMaster pxMBHdl );
...
pxMBMGetNewHdl( pxMBMaster pxMBHdl )
{
    ...
    for( ubIdx = 0; ubIdx < MB_UTILS_NARRSIZE( xMBMInternalHdl ); ubIdx++ )
    {
        if( IDX_INVALID == xMBMInternalHdl[ubIdx].ubIdx )
        {
            ...
            pxIntHdl->xTmr_RespTimeoutHdl = pxMBHdl->xTmrTimeoutHdl; 
#if MBM_TIMEOUT_MODE_AFTER_TRANSMIT == 1
...
eMBMSerialInit( pxMBMaster pxMBHdl, eMBSerialMode eMode, UCHAR ucPort, ULONG ulBaudRate, eMBSerialParity eParity )
{
    ...
    eStatus = eMBMSerialInitExt( pxMBHdl, eMode, ucPort, ulBaudRate, eParity, ucStopBits );
    return eStatus;
}
                   ...
                   UCHAR ucStopBits )
{
    ...
    xMBMHandle          *pxHdl = &(pxMBHdl->xMBMHdl);
#if MBP_ADVA_STARTUP_SHUTDOWN_ENABLED == 1
    MBP_ENTER_CRITICAL_INIT();
    if( 0 == ubMBMCountInstances() )
    {
    ...
    if( NULL != pxHdl )
    {
        if( NULL == ( pxMBMNewIntHdl = pxMBMGetNewHdl(pxMBHdl) ) )
        {
            eStatus = MB_ENORES;
        }
        else
        {
            switch( eMode )
            {
#if MBM_ASCII_ENABLED == 1
                case MB_ASCII:
                    pxMBMNewIntHdl->rs485_de_deselect = pxMBHdl->rs485_de_deselect;
                    pxMBMNewIntHdl->rs485_de_select = pxMBHdl->rs485_de_select;
                    pxMBMNewIntHdl->xSer_Hdl = pxMBHdl->xSerHdl;
                    pxMBMNewIntHdl->xTmr_Hdl = pxMBHdl->xTmrHdl;
    #if MBM_ASCII_BACKOF_TIME_MS > 0
                    pxMBMNewIntHdl->xTmr_BackOffHdl = pxMBHdl->xTmr_BackOffHdl;
    #endif
                    pxMBMNewIntHdl->xIsRTU = FALSE;
                    pxMBMNewIntHdl->xIsASCII = TRUE;
    #if MBM_ASCII_WAITAFTERSEND_ENABLED == 1
                    pxMBMNewIntHdl->xTmrWaitHdl = pxMBHdl->xTmrWaitHdl;
    #endif
                ...
                case MB_RTU:
                    ...
                    pxMBMNewIntHdl->xSer_Hdl = pxMBHdl->xSerHdl;
                    pxMBMNewIntHdl->xTmr_Hdl = pxMBHdl->xTmrHdl;
                    pxMBMNewIntHdl->xIsRTU = TRUE;
                    pxMBMNewIntHdl->xIsASCII = FALSE;
