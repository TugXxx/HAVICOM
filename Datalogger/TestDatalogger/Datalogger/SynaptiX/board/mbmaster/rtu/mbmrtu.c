...
STATIC BOOL
bMBMSerialRTUTransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE *ubValue );
#elif MBM_SERIAL_API_VERSION == 2
STATIC BOOL
bMBMSerialRTUTransmitterEmptyAPIV2CB( xMBHandle xHdl, UBYTE *pubBufferOut, USHORT usBufferMax,
                                      USHORT *pusBufferWritten );
STATIC void
...
void
vMBMLogRTUFrame( eMBPortLogLevel eLevel, xMBMInternalHandle *pxIntHdl, char *szMsg, const UBYTE *pubPayload,
                 USHORT usLength );
#endif
                   ...
                   UCHAR ucStopBits )
{
...
#endif
    {
	    ...
	    /* If you feel the need to modify the usTimeoutMS value because your slave behave strangly consider MBP_SERIAL_PORT_DETECTS_TIMEOUT
	       an demo showing how to do it is STM32-L072_CUBEMX_FREERTOS
               it would be fine to modify the value right here, you only need to be careful when updating the core stack */
#endif
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_OTHER,
                             "[IDX=" MBP_FORMAT_USHORT "] Using RTU timeout of " MBP_FORMAT_USHORT "ms.\n",
...
#endif
            /* Below, pxFrameHdl is assigned, because the mbporttimmer.c needs to 
            detect the appropriate handle to initialize  */
            pxIntHdl->xFrameHdl = pxFrameHdl;
            if( MB_ENOERR != ( eStatus2 = eMBPSerialInit( &( pxFrameHdl->xSerHdl ), ucPort, ulBaudRate, 8, eParity,
                                                          ucStopBits, pxIntHdl
            ...
            else if( MB_ENOERR != ( eStatus2 = eMBPTimerInit( &( pxFrameHdl->xTmrHdl ), usTimeoutMS, bMBMSerialRTUT35CB,
                                                              pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
#if MBM_RTU_WAITAFTERSEND_ENABLED == 1
            else if( MB_ENOERR != ( eStatus2 = eMBPTimerInit( &( pxFrameHdl->xTmrWaitHdl ), usTimeoutMSWaitAfterSend,
                                                              bMBMSerialWaitCB, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
#endif
            ...
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_RTU,
                             ...
                             ( USHORT )pxFrameHdl->ubIdx, ( USHORT )ucPort, ulBaudRate, ( USHORT )eParity,
...
#endif
    {
        ...
        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
        {
            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_RTU,
                         ...
                         " with length=" MBP_FORMAT_USHORT ".\n",
                         ( USHORT )pxRTUHdl->ubIdx, ( USHORT )ucSlaveAddress, usMBPDULength );
        }
#endif
        if( MB_IS_VALID_HDL( pxRTUHdl, xMBMRTUFrameHdl ) && MB_IS_VALID_WRITE_ADDR( ucSlaveAddress ) &&
            ( usMBPDULength <= ( MBM_SER_PDU_SIZE_MAX - ( 1 /* Slave Address */ + 2 /* CRC16 */ ) ) ) )
        {
            MBP_ENTER_CRITICAL_SECTION();
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_RTU,
                             ...
                             ", pos = " MBP_FORMAT_USHORT " ), sender ( state = " MBP_FORMAT_USHORT
                             ", cnt = " MBP_FORMAT_USHORT " ).\n",
                             ( USHORT )pxRTUHdl->ubIdx, ( USHORT )pxRTUHdl->eRcvState, pxRTUHdl->usRcvBufferPos,
                             ( USHORT )pxRTUHdl->eSndState, pxRTUHdl->usSndBufferCnt );
            }
#endif
...
#if MBM_SERIAL_API_VERSION == 2
            if( MB_ENOERR !=
                ( eStatus = eMBPSerialTxEnable(
                      pxRTUHdl->xSerHdl, ( pbMBPSerialTransmitterEmptyCB )bMBMSerialRTUTransmitterEmptyAPIV2CB, pxIntHdl ) ) )
            {
            ...
            if( MB_ENOERR !=
                ( eStatus = eMBPSerialTxEnable(
                      pxRTUHdl->xSerHdl, ( pbMBPSerialTransmitterEmptyCB )bMBMSerialRTUTransmitterEmptyAPIV1CB, pxIntHdl ) ) )
            {
                HDL_RESET_TX( pxRTUHdl );
            }
#else
            ...
            else
            {
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
                {
                    ...
                    vMBMLogRTUFrame( MB_LOG_DEBUG, pxIntHdl, "Sending frame: ",
                                     ( const UBYTE * )&pxRTUHdl->ubRTUFrameBuffer[MBM_SER_PDU_ADDR_OFF],
                                     pxRTUHdl->usSndBufferCnt );
#else
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_RTU,
...
eMBMSerialRTUFrameReceive( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT *pusMBPDULength )
{
...
#endif
    {
        ...
        if( MB_IS_VALID_HDL( pxRTUHdl, xMBMRTUFrameHdl ) && MB_IS_VALID_READ_ADDR( ucSlaveAddress ) )
        {
            ...
            {
                *pusMBPDULength =
                    ( USHORT )( pxRTUHdl->usRcvBufferPos - ( MBM_SER_PDU_PDU_OFF + MBM_SER_PDU_SIZE_CRC ) );
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
                {
#if defined( MBM_ENABLE_DEBUG_FACILITY_HEAVY ) && ( MBM_ENABLE_DEBUG_FACILITY_HEAVY == 1 )
            ...
            else
            {
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_RTU ) )
                {
                    ...
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_RTU,
                                 ...
                                 "] Received frame with incorrect checksum or length " MBP_FORMAT_USHORT "!\n",
                                 ( USHORT )pxRTUHdl->ubIdx, pxRTUHdl->usRcvBufferPos );
#endif
                }
#endif
...
#endif
            MBP_ENTER_CRITICAL_SECTION();
            if( MB_ENOERR != ( eStatus2 = eMBPSerialTxEnable( pxRTUHdl->xSerHdl, NULL, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
            if( MB_ENOERR != ( eStatus2 = eMBPSerialRxEnable( pxRTUHdl->xSerHdl, NULL , pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
...
bMBMSerialRTUReceiverAPIV1CB( xMBHandle xHdl, UBYTE ubValue )
{
    ...
    if( MB_ENOERR != eMBPTimerStart( pxRTUFrameHdl->xTmrHdl ) )
    {
        ...
        eStatus = eMBPSerialRxEnable( pxRTUFrameHdl->xSerHdl, NULL , pxIntHdl );
...
bMBMSerialRTUTransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE *pubValue )
{
    ...
    switch( pxRTUFrameHdl->eSndState )
    {
             ...
             * MBM_EV_SENT event to the main state machine. We need to wait for an interframe timeout.
             * Otherwise we enable the receiver or abort using an error.
             */
    ...
    if( !bMoreTXData )
    {
        ...
        if( bEnterTxWait )
        {
            pxRTUFrameHdl->eSndState = MBM_STATE_TX_WAIT;
            eStatus = eMBPTimerStart( pxRTUFrameHdl->xTmrHdl );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
    }
    else
...
#else
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxRTUFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB )bMBMSerialRTUReceiverAPIV1CB ) , pxIntHdl )
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType )MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
                                      ...
                                      USHORT *pusBufferWritten )
{
    ...
    switch( pxRTUFrameHdl->eSndState )
    {
        ...
        case MBM_STATE_TX_XMIT:
            ...
            {
                usBytesToSend =
                    pxRTUFrameHdl->usSndBufferCnt < usBufferMax ? pxRTUFrameHdl->usSndBufferCnt : usBufferMax;
...
#else
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxRTUFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB )vMBMSerialRTUReceiverAPIV2CB , pxIntHdl ) )
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType )MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
...
bMBMSerialRTUT35CB( xMBHandle xHdl )
{
    ...
    if( pxRTUFrameHdl->eSndState == MBM_STATE_TX_WAIT )
    {
        /* after an IFT we can report the broadcast frame to be sent */
        HDL_RESET_TX( pxRTUFrameHdl );
